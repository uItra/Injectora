...
int TableHeaderComponent::getColumnHeight(const int columnId) const
{
	if (const ColumnInfo* const ci = getInfoForId(columnId))
		return ci->height;
...
void TableHeaderComponent::setColumnHeight(const int columnId, const int newHeight)
{
	ColumnInfo* const ci = getInfoForId(columnId);

	if (ci != nullptr && ci->height != newHeight)
	{
		ci->height = newHeight;
		repaint();
		columnsResized = true;
    ...
    for (int i = 0; i < columns.size(); ++i)
    {
        ...
        if (ci->isVisible())
        {
                      ...
                      || ! dragOverlayComp->isVisible()))
            {
                ...
                g.setOrigin (x, 0);
                g.reduceClipRegion (0, 0, ci->width, ci->height /*getHeight()*/);

				lf.drawTableHeaderColumn(g, ci->name, ci->id, ci->width, ci->height /*getHeight()*/,
